      ...
      {(searchMode === 'classic' || isMultiDestination) && (
        <div style={{ maxWidth: 900, margin: '-20px auto 0', padding: '0 24px 60px' }}>
          ...
          {status === 'done' && totalFound === 0 && (() => {
            ...
            const KIWI_AFF = 'travelpayoutsdeeplink_www.golivoo.com.br_a576819c844940aeb1161c401-715532'
            const kiwiUrl = orig && dest
              ? (() => {
                  const p = new URLSearchParams({ affilid: KIWI_AFF, from: orig.toUpperCase(), to: dest.toUpperCase(), lang: 'br', currency: 'BRL' })
                  if (date && date.length === 10) { const [y,m,d] = date.split('-'); p.set('departure', `${d}-${m}-${y}`) }
                  return `https://www.kiwi.com/deep?${p.toString()}`
                })()
            ...
            return (
              <div style={{ background: '#fff', borderRadius: 14, padding: 48, textAlign: 'center', boxShadow: '0 4px 20px rgba(13,27,62,0.07)' }}>
                ...
                <p style={{ fontFamily: 'Inter, sans-serif', color: '#64748B', fontSize: '0.9rem', marginBottom: 28, maxWidth: 460, margin: '0 auto 28px' }}>
                  ...
                  Continue sua busca no Kiwi.com com preços em tempo real, em Português e Reais.
                </p>
                <a
                  href={kiwiUrl}
                  target="_blank"
          ...
          {status === 'done' && !isMultiDestination && totalFound > 0 && (
            <>
              ...
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.78rem', color: '#64748B', textAlign: 'center', marginTop: 8 }}>
                Preços em R$ (BRL) · Via Travelpayouts/Kiwi.com · Sem markup · Sujeito a disponibilidade
              </p>
            </>
          )}
