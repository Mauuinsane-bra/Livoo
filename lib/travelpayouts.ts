...
// Travelpayouts / Aviasales API — precos reais, sem markup, gratis para afiliados
// Docs: https://travelpayouts.github.io/slate/
//
// Autocomplete: https://autocomplete.travelpayouts.com  -> SEM autenticacao
...
export interface FlightSearchResult {
  ...
  arrivalTime:   string   // "" -- Data API nao retorna
  duration:      string   // "" -- Data API nao retorna
  stops:         number
  price:         number
  currency:      string
  link:          string   // link de compra no Kiwi.com (afiliado Travelpayouts)
}
...
// -- Airport Search -- SEM autenticacao
...
// Retorna precos cacheados (atualizados a cada poucas horas)

export async function searchFlights(params: SearchFlightsParams): Promise<FlightSearchResult[]> {
  const token = getToken()
  if (!token) throw new Error('TRAVELPAYOUTS_TOKEN nao configurado')
...
// -- Transformacao do retorno da API --
  ...
  for (const [destCode, flights] of Object.entries(rawData)) {
    for (const [, flight] of Object.entries(flights)) {
      ...
      results.push({
        ...
        link:          buildKiwiUrl(params.origin, destCode, params.date),
      })
    }
  }
...
// -- Link de compra Kiwi.com (afiliado Travelpayouts) --
// Deeplink via /deep endpoint: preenche origem, destino e data automaticamente
// Affiliate tracking ativo via affilid param (Travelpayouts)
